...
// Important: Resend requires a verified domain to send emails to arbitrary users.
// If you haven't verified a domain on Resend, you must use "onboarding@resend.dev"
// and you can only send emails to your own email address.
...
async function sendCustomerEmail(customerEmail, customerName, driver, load) {
    ...
    try {
        const { data, error } = await resend.emails.send({
            from: `"DeadTrip Hunter" <${FROM_EMAIL}>`,
            to: customerEmail,
...
async function sendRideAcceptedEmail({ customer, driver, ride, sourceAddress, destinationAddress }) {
    ...
    try {
        ...
        const driverRes = await resend.emails.send({
            from: `"DeadTrip Hunter" <${FROM_EMAIL}>`,
            to: driver.email,
